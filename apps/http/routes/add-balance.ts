import { addBalanceSchema, HTTP_BACKEND_STREAM_CONFIGS, ORDER_ENGINE_STREAM_CONFIGS, zodErrorMessage, type EngineResponse } from "@repo/common/common";
import { redisManager } from "@repo/redis/redis";
import type { Request, Response } from "express";

export async function addBalance(req: Request, res: Response) {  
  const { success, data, error } = addBalanceSchema.safeParse(req.body);

  if (!success) {
    return res.json({ message: zodErrorMessage({ error }) })
  }
